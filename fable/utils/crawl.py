...
class ProxySelector:
    ...
    def select_url(self, scheme="http"):
        ...
        if proxy == {}:
            ...
            return "{}://{}".format(scheme, proxy[scheme])
...
class RobotParser:
    ...
    def __init__(self, useragent=requests_header["user-agent"]):
        ...
        self.rp = RobotsCache(
            capacity=1000, ttl_policy=policy, headers=requests_header, timeout=10
        )
    ...
    def allowed(self, url, useragent=None):
        ...
        if robot_url not in self.req_status:
            try:
                r = requests.get(
                    robot_url, timeout=5, headers={"user-agent": useragent}
                )
...
def chrome_crawl(url, timeout=120, screenshot=False, ID="", proxy=None):
    ...
    try:
        ...
        cmd = [
            "node",
            join(dirname(abspath(__file__)), "run.js"),
            f'"{url}"',
            "--filename",
            cur,
        ]
...
def wayback_year_links(
    prefix, years, NUM_THREADS=3, max_limit=0, param_dict={}, proxies={}
):
    """
...
def requests_crawl(url, timeout=20, wait=True, html=True, proxies={}, raw=False):
    ...
    while True:
        try:
            ...
            if (
                wait and (r.status_code == 429 or r.status_code == 504) and count < 3
            ):  # Requests limit
            ...
            if len(proxies):
                logger.warn(
                    f"Connection Error with Proxies: {str(exc)},\n Retry without proxy"
                )
                proxies = {}
            else:
                logger.warn(
                    f"There is an ConnectionError exception with requests_crawl"
                )
                return
...
def get_sitemaps(hostname):
    ...
    try:
        r = requests.get(
            "http://{}/".format(hostname), headers=requests_header, timeout=10
        )
    except:
        return None
...
def __extract_breadcrumb(tag):
    cur_tag = tag
    while cur_tag:
        neighbor_a = cur_tag.find_previous_siblings("a") + cur_tag.find_next_siblings(
            "a"
        )
        if len(neighbor_a) > 0:
...
def _breadcrumb_vague(url, html, wayback):
    ...
    for ancestor in ancestors:
        ...
        atags += [
            ...
            if re.compile(q + "$").search(a.get("href"))
        ]
    # print(atags)
